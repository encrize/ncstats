...
D[an snapshot of your Neocities site stats and appends it to data/stats.json.
...
Data collected:
  ...
  - Private list (needs API key): file count, total size in bytes, last file update
...
Env vars:
  NEOCITIES_SITENAME   required, e.g. "jaryq"
  NEOCITIES_API_KEY    optional; if set, also pulls /api/list (files + size)
"""
...
USER_AGENT = "neocities-stats-dashboard/1.0 (+github actions)"
...
def fetch_files(api_key: str) -> dict:
    """Returns {count, total_bytes, last_file_update} or {} on failure."""
    try:
